...
/** 
* @Description: 策略分类controller（使用VO类，不直接出现实体类）
* @author tphe06
* @date 2017年10月17日 下午1:57:50  
*/
@Controller
@RequestMapping("${adminPath}/strategyType")
public class StrategyTypeController extends BaseController {
	private static Log log = LogFactory.getLog(StrategyTypeController.class);

	@Autowired
	private IStrategyTypeService service;
	...
	@ResponseBody
    public ResultModel list(@Validated(value = { ValidSearch.class }) @RequestBody StrategyTypeVo vo, BindingResult br) {
		try {
			if ( br.hasErrors() ) {
				return super.handleValid(br);
			}
			Principal p = LoginUtil.getLoginUser();
			List<StrategyTypeVo> list = service.list(vo, p);
	...
	@ResponseBody
    public ResultModel select(@Validated(value = { ValidSearch.class }) @RequestBody StrategyTypeVo vo, BindingResult br) {
		try {
			if ( br.hasErrors() ) {
				return super.handleValid(br);
			}
			Principal p = LoginUtil.getLoginUser();
			List<StrategyTypeVo> list = service.select(vo, p);
	...
	@ResponseBody
	public ResultModel view(@RequestBody StrategyTypeVo vo)  {
       try {
    	   StrategyTypeVo v = service.view(vo.getEntity().getStrategyId());
	...
	@ResponseBody
	public ResultModel count(@Validated(value = { ValidSearch.class }) @RequestBody StrategyTypeVo vo, BindingResult br) {
       try {
		   if ( br.hasErrors() ) {
			   return super.handleValid(br);
		   }
	...
	@ResponseBody
	public ResultModel insert( @Validated(value = { ValidSave.class }) @RequestBody StrategyTypeVo vo, BindingResult br) {
		try {
			if ( br.hasErrors() ) {
				return super.handleValid(br);
			}
	...
	@ResponseBody
	public ResultModel update(@Validated(value = { ValidUpdate.class }) @RequestBody StrategyTypeVo vo, BindingResult br) {
		try {
			if ( br.hasErrors() ) {
				return super.handleValid(br);
			}
	...
	@ResponseBody
	public ResultModel delete(@RequestBody StrategyTypeVo vo) {
		try {

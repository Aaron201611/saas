package com.yunkouan.saas.common;
...
@ContextConfiguration({ "classpath*:spring/spr-context.xml", "classpath*:spring/spring-mvc.xml",
...
public class BaseJunitTest implements ApplicationContextAware {
	 ...
	 * @author andy wang<br/>
	 */
	protected void formatResult( BaseEntity be ) throws Exception {
	...
	protected void formatResult(ResultModel rm) {
		...
		try {
			...
			if (rm.isSuccess()) {
				...
				if (page != null) {
					sb.append(String.format("当前【%s】页,【%s】数据\n", page.getPageNum(), page.getTotal()));
